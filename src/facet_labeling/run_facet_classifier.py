...
from helper.cskb_logging import print_log
...
def main():
    ...
    # Load RoBERTa
    print_log("Loading RoBERTa model...")
    ...
    if args.do_predict:
        print_log("Creating prediction data loaders")
        ...
        print_log("Running classification for {} samples...".format(len(test_set)))
        ...
        if test_set.has_label:
            print_log("Test Accuracy: {:.2f}%".format(eval_accuracy * 100))

    elif args.do_train:
        print_log("Creating training data loaders")
        train_set = FacetDataset(filename=train_file, tokenizer=tokenizer, maxlen=args.max_seq_length)
        train_loader = DataLoader(train_set, batch_size=args.batch_size, num_workers=args.num_workers, shuffle=True)

        val_loader = None
        if args.do_eval:
            print_log("Creating validation data loaders")
        ...
        # Save model
        print_log("Saving model to {}".format(output_dir))
